...
/** (Re)Calculate and return the current potential energy of the system.
 *
\verbatim embed:rst

This function forces the recalculation of all energies (and forces),
regardless of the last timestep and everything else, and returns the freshly
calculated total energy.
In order to maximally reduce overhead only the global energy and no virials
are calculated and atom coordinates and the box are assumed unchanged.
...
is expected to) continue afterwards. Using :doc:`run 0 post no` (pre is needed)
results in a lot of unnecessary overhead (logging, various setups etc.).

\endverbatim
 *
 * \param  handle       pointer to a previously created LAMMPS instance
 * \param  neigh_flag   if nonzero neighbor lists are rebuilt before calculation
 * \return              the newly calculated potential energy of the system */
